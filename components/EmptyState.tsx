...
export function EmptyState({ icon: Icon, title, subtitle, actionLabel, onAction }: EmptyStateProps) {
  return (
    <View className="items-center justify-center px-8 py-16">
      ...
      <Text className="text-text-primary text-lg font-bold text-center mb-1.5">{title}</Text>
      {subtitle && (
        <Text className="text-text-secondary text-sm text-center leading-5 mb-6">{subtitle}</Text>
      )}
      {actionLabel && onAction && (
        <PrimaryButton label={actionLabel} onPress={onAction} className="w-full max-w-[240px]" />
      )}
    </View>
  );
}