...
export function FilterChip({ label, selected, onPress }: ChipProps) {
  return (
    <Pressable
      ...
      accessibilityRole="button"
      accessibilityState={{ selected: !!selected }}
      className={`px-3.5 py-2 rounded-lg mr-2 ${selected ? 'bg-secondary' : 'bg-surface-alt'}`}
    >
      <Text className={`text-sm font-semibold ${selected ? 'text-bg' : 'text-text-secondary'}`}>{label}</Text>
    </Pressable>
  );
}
...
export function TagChip({ label, onRemove }: { label: string; onRemove?: () => void }) {
  return (
    <Pressable
      onPress={onRemove}
      className="flex-row items-center bg-surface-alt border border-border rounded-lg pl-3 pr-2 py-1.5 mr-2 mb-2"
    >
